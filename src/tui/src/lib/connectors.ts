...
function baseTargetLabel(
  target?: Pick<ConnectorTargetSnapshot, 'label' | 'chat_type' | 'chat_id' | 'conversation_id' | 'profile_label'> | null
) {
  if (!target) return ''
...
function mergeTargetEntry(
  ...
  options: { defaultConversationId: string | null }
) {
  ...
  const nextSources = Array.from(
    new Set(
      [...(existing.sources || []), ...(normalizedTarget.sources || []), existing.source, normalizedTarget.source].filter(

...
- Tags: `multiphysics`, `finite-element`, `cfd`, `structural-mechanics`, `fsi`, `hpc`
- Knowledge URL: https://github.com/skilled-scipkg/Kratos
- Source archive URL: https://github.com/skilled-scipkg/Kratos/archive/refs/heads/master.zip
- Upstream project URL: https://github.com/KratosMultiphysics/Kratos
...
Kratos Multiphysics is a modular, parallel framework for coupled finite-element simulations across fluid dynamics, structural mechanics, contact, DEM, and fluid-structure interaction domains.
...
```bash
command -v kratos || true
kratos --version || true
```

If the package is available only through environment modules, record the module
state and the exact executable path in `validation/environment/kratos_doctor.json`.
...
Generated import or executable names are starting points. If `kratos` uses

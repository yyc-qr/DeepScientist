# ParFlow Watershed Flow Model

## Catalog

- Package id: `parflow`
- Domains: `workflow_provenance`
- Tags: `hydrology`, `watershed`, `groundwater`, `surface-water`, `hpc`, `environmental-modeling`
- Knowledge URL: https://github.com/skilled-scipkg/parflow
- Source archive URL: https://github.com/skilled-scipkg/parflow/archive/refs/heads/master.zip
- Upstream project URL: https://github.com/parflow/parflow
- Homepage: http://parflow.org
- Catalog source: FermiLink skilled-scipkg, commit `93f089a333a43089fb1a08a73c37d05fd6683214`

## When To Consider

ParFlow is a massively parallel integrated hydrologic model for coupled surface and subsurface watershed flow, simulating overland runoff, groundwater, and land-surface processes across complex terrain.
...
```bash
command -v parflow || true
parflow --version || true
```

If the package is available only through environment modules, record the module
state and the exact executable path in `validation/environment/parflow_doctor.json`.
...
Generated import or executable names are starting points. If `parflow` uses

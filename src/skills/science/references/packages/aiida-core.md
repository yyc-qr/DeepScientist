# AiiDA Core Workflow Engine

## Catalog

- Package id: `aiida-core`
- Domains: `workflow_provenance`
- Tags: `workflow`, `provenance`, `hpc`, `computational-science`, `reproducibility`, `scientific-automation`
- Knowledge URL: https://github.com/skilled-scipkg/aiida-core
- Source archive URL: https://github.com/skilled-scipkg/aiida-core/archive/refs/heads/main.zip
- Upstream project URL: https://github.com/aiidateam/aiida-core
...
AiiDA-core is a scientific workflow and provenance platform that automates remote HPC calculations, tracks reproducible process graphs, and integrates simulation-code plugins.
...
```bash
command -v aiida-core || true
aiida-core --version || true
```

If the package is available only through environment modules, record the module
state and the exact executable path in `validation/environment/aiida-core_doctor.json`.
...
Generated import or executable names are starting points. If `aiida-core` uses

# pyiron Base Workflow Core

## Catalog

- Package id: `pyiron_base`
- Domains: `materials_science`, `workflow_provenance`
- Tags: `computational-materials-science`, `workflow-management`, `hpc`, `job-orchestration`, `hdf5`, `parameter-studies`
- Knowledge URL: https://github.com/skilled-scipkg/pyiron_base
- Source archive URL: https://github.com/skilled-scipkg/pyiron_base/archive/refs/heads/main.zip
- Upstream project URL: https://github.com/pyiron/pyiron_base
...
pyiron_base provides core workflow, job orchestration, and HDF5-based data management for large-scale computational materials science studies across local and HPC resources.
...
```bash
command -v pyiron_base || true
pyiron_base --version || true
```

If the package is available only through environment modules, record the module
state and the exact executable path in `validation/environment/pyiron_base_doctor.json`.
...
Generated import or executable names are starting points. If `pyiron_base` uses

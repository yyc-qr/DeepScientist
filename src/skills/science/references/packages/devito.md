...
- Tags: `pde`, `finite-difference`, `stencil`, `symbolic-dsl`, `hpc`, `seismic`
- Knowledge URL: https://github.com/skilled-scipkg/devito
- Source archive URL: https://github.com/skilled-scipkg/devito/archive/refs/heads/main.zip
- Upstream project URL: https://github.com/devitocodes/devito
...
Devito is a symbolic Python DSL and compiler framework that generates optimized finite-difference and stencil operators for PDE simulations on CPUs, GPUs, and distributed-memory clusters.
...
```bash
command -v devito || true
devito --version || true
```

If the package is available only through environment modules, record the module
state and the exact executable path in `validation/environment/devito_doctor.json`.
...
Generated import or executable names are starting points. If `devito` uses

...
- Tags: `cfd`, `hpc`, `gpu`, `spectral-elements`, `navier-stokes`, `fluid-dynamics`
- Knowledge URL: https://github.com/skilled-scipkg/nekRS
- Source archive URL: https://github.com/skilled-scipkg/nekRS/archive/refs/heads/master.zip
- Upstream project URL: https://github.com/Nek5000/nekRS
...
nekRS is a GPU-accelerated high-order spectral-element computational fluid dynamics solver for incompressible and low-Mach Navier-Stokes flows on massively parallel HPC systems.
...
```bash
command -v nekrs || true
nekrs --version || true
```

If the package is available only through environment modules, record the module
state and the exact executable path in `validation/environment/nekrs_doctor.json`.
...
Generated import or executable names are starting points. If `nekrs` uses

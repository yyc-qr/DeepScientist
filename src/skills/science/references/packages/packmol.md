# Packmol Molecular Configuration Builder

## Catalog

- Package id: `packmol`
- Domains: `computational_chemistry`, `molecular_dynamics`
- Tags: `molecular-dynamics`, `molecule-packing`, `initial-configuration`, `computational-chemistry`, `structure-preparation`
- Knowledge URL: https://github.com/skilled-scipkg/packmol
- Source archive URL: https://github.com/skilled-scipkg/packmol/archive/refs/heads/master.zip
- Upstream project URL: https://github.com/m3g/packmol
- Homepage: http://m3g.github.io/packmol
- Catalog source: FermiLink skilled-scipkg, commit `93f089a333a43089fb1a08a73c37d05fd6683214`

## When To Consider

Packmol generates initial molecular dynamics coordinates by packing molecules into user-defined regions while satisfying minimum-distance constraints to prevent destabilizing atomic overlaps.
...
```bash
command -v packmol || true
packmol --version || true
```

If the package is available only through environment modules, record the module
state and the exact executable path in `validation/environment/packmol_doctor.json`.
...
Generated import or executable names are starting points. If `packmol` uses

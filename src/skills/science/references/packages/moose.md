...
- Tags: `multiphysics`, `finite-element`, `pde`, `simulation`, `hpc`, `engineering`
- Knowledge URL: https://github.com/skilled-scipkg/moose
- Source archive URL: https://github.com/skilled-scipkg/moose/archive/refs/heads/next.zip
- Upstream project URL: https://github.com/idaholab/moose
...
MOOSE is an object-oriented finite-element framework for fully coupled implicit multiphysics simulations with adaptive meshing, scalable parallel solvers, and engineering-focused APIs.
...
```bash
command -v moose || true
moose --version || true
```

If the package is available only through environment modules, record the module
state and the exact executable path in `validation/environment/moose_doctor.json`.
...
Generated import or executable names are starting points. If `moose` uses

...
function makeDraftId() {
  return `setup-draft-${Date.now()}-${Math.random()
    .toString(16)
    .slice(2)}`
}
...
function copy(locale: 'en' | 'zh') {
  return locale === 'zh'
    ? {
        ...
        placeholder: '请帮我找一个适合本机的 LLM 方向的任务',
        cta: '启动协助',
        note:
          '启动后，右侧聊天面板会继续帮你整理内容，并把建议直接写回左侧表单。',
      }
    : {
        title: 'SetupAgent',
        intro:
        ...
        cta: 'Start Assist',
        note:
          'Once started, the chat panel on the right keeps refining the launch form and writes suggestions back into the left form.',
      }
}
...
export function SetupAgentRail(props: SetupAgentRailProps) {
  ...
  const handleQueueFiles = React.useCallback(
    (files: File[]) => {
  ...
  return (
    <div className="ai-manus-root ai-manus-copilot ai-manus-embedded flex h-full min-h-0 flex-col overflow-hidden rounded-[24px] border border-[#30466F] bg-[#0B1730]/95 shadow-[0_24px_70px_-54px_rgba(0,0,0,0.78)] backdrop-blur-xl">

      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-[#30466F] px-4 py-3">
        <div>
          <div className="text-sm font-semibold text-[#EAF2FF]">
            {t.title}
          </div>

          <div className="mt-1 text-xs text-[#9FB2D3]">
            {props.setupPacket?.assistant_label ||
              props.assistantLabel ||
              t.title}
          </div>
        </div>

        <div className="inline-flex items-center gap-2 rounded-full border border-[#38527F]/60 bg-[#12213F] px-3 py-1 text-[11px] font-medium text-[#BFD3F4]">
          <Sparkles className="h-3.5 w-3.5 text-[#8FB4FF]" />
      ...
      <div className="feed-scrollbar modal-scrollbar min-h-0 flex-1 overflow-y-auto px-4 py-4">
        <div className="space-y-4 pr-1">
          <div className="rounded-[18px] border border-[#30466F] bg-[#14213D]/80 px-4 py-4 text-[15px] leading-7 text-[#BFD3F4]">
            {props.setupPacket
              ? t.benchmarkIntro
              : t.intro}
          </div>
      ...
      <div className="shrink-0 border-t border-[#30466F] px-4 py-4">
        <div className="space-y-3">

          <div className="text-sm leading-6 text-[#AFC1DF]">
            {t.note}
          </div>
          ...
          <QuestCopilotComposer
            ...
            enterHint={
              props.locale === 'zh'
                ? '可直接拖入文件 · Enter 发送 · Shift+Enter 换行'
                : 'Drop files directly · Enter to send · Shift+Enter for newline'
            }
            sendLabel={t.cta}
            stopLabel={
              props.locale === 'zh'

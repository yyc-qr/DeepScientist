...
export const COUNTRY_OPTIONS: CountryOption[] = [
  ...
  { code: 'TW', label: 'Taiwan, Province of PRC' },
...
COUNTRY_OPTIONS.forEach((country) => {
  COUNTRY_CODE_LOOKUP.set(country.code.toLowerCase(), country.code)
  COUNTRY_CODE_LOOKUP.set(country.label.toLowerCase(), country.code)
})
Object.entries(COUNTRY_ALIASES).forEach(([label, code]) => {
  COUNTRY_CODE_LOOKUP.set(label, code)
})
...
export const normalizeCountryCode = (value?: string | null): string => {
  ...
  return COUNTRY_CODE_LOOKUP.get(normalized) ?? ''
}

export const getCountryFlagEmoji = (countryCode: string): string => {
  if (!countryCode) return ''
  const normalized = countryCode.trim().toUpperCase()
  const resolved = COUNTRY_FLAG_OVERRIDES[normalized] ?? normalized
  if (!/^[A-Z]{2}$/.test(resolved)) return ''
  const offset = 0x1f1e6 - 65
  return String.fromCodePoint(
    resolved.charCodeAt(0) + offset,
    resolved.charCodeAt(1) + offset
  )
}

export const getCountryFlagImageUrl = (countryCode: string, size: 16 | 20 | 24 = 20): string => {
  if (!countryCode) return ''
  const normalized = countryCode.trim().toUpperCase()
  const resolved = COUNTRY_FLAG_OVERRIDES[normalized] ?? normalized
  if (!/^[A-Z]{2}$/.test(resolved)) return ''
  const height = Math.round((size * 3) / 4)
  return `https://flagcdn.com/${size}x${height}/${resolved.toLowerCase()}.png`
}

export const formatCountryOptionLabel = (country: CountryOption): string => {

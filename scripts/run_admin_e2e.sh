...
RUNTIME_DIR="${E2E_RUNTIME_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/ds-admin-e2e.XXXXXX")}"
...
UI_TEST_SCRIPT="${E2E_UI_TEST_SCRIPT:-}"
PLAYWRIGHT_SPEC="${E2E_PLAYWRIGHT_SPEC:-$ROOT_DIR/src/ui/e2e/settings-control-center.spec.ts}"
...
python "$ROOT_DIR/scripts/setup_admin_e2e_fixture.py" \
  --home "$HOME_DIR" \
  --output "$FIXTURE_JSON"
...
if [[ -n "$UI_TEST_SCRIPT" ]]; then
  E2E_BASE_URL="$BASE_URL" \
  E2E_FIXTURE_JSON="$FIXTURE_JSON" \
  npm --prefix "$ROOT_DIR/src/ui" run "$UI_TEST_SCRIPT" -- --reporter=list --project="$PLAYWRIGHT_PROJECT"
else
  E2E_BASE_URL="$BASE_URL" \
  E2E_FIXTURE_JSON="$FIXTURE_JSON" \

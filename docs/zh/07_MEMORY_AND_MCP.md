...
- 用 memory 卡片同步或重建当前 quest 的知识图谱

使用场景：

- 写入卡片后、希望图检索能看到这些卡片时
- 只有需要从零重建图时才用 `rebuild=True`

图是卡片之上的增量检索层：卡片始终是权威持久化，图可随时重建。
...
- Dense（QWEN embedding）+ BM25 -> RRF 融合 -> 锚点节点
- BFS 子图展开（1-2 跳）
- QWEN re-rank -> contradicts 降权 + 时间衰减 -> Top-k

使用场景：

- 查找发现演化链、演化/矛盾关系和已知失败模式
- 在重试之前查询历史失败记录

节点 embedding 缓存在 `embeddings.json`；首次检索时缺失的节点 embedding 会按需生成。

### `memory.classify_failure(...)`

用途：

- 用 QWEN 对失败的实验进行分类，并写入知识图谱
...
- 总是记录 Experiment 节点并带上失败类别
- `hypothesis_invalid` 会把 Idea 标记为 `dead_end`，并通过 `fails_with` 边连接 Failure 节点
- 其他类别只停留在 episode 层，不创建知识层的失败节点
